...
var OmxManager = function() {
    ...
    for (var action in _commands) {
        if (_commands.hasOwnProperty(action)) {
            (function(key) {
                OmxManager.prototype[action] = function() {
...
OmxManager.prototype = Object.create(EventEmitter.prototype);
OmxManager.prototype.constructor = OmxManager;

OmxManager.prototype.toString = function() {
    if (_omxProcess) {
        return _omxProcess.pid;
    }
 ...
 */
OmxManager.prototype.stop = function() {
 ...
 */
OmxManager.prototype.pause = function() {
    if (_paused) return;
    /* ignore if process isn't running */
    if (!_omxProcess) return;
 ...
 * @fires OmxManager#load
 */
OmxManager.prototype.play = function(videos, args, loop) {
    if (_omxProcess) {
        if (_paused) {
            _sendAction('p');
            if (_handleHanging) {
 ...
 */
OmxManager.prototype.enableMultipleNativeLoop = function() {
 ...
 */
OmxManager.prototype.enableHangingHandler = function() {
    _handleHanging = true;
};
